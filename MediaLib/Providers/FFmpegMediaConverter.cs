...
using StreamType = MediaLib.FFmpeg.StreamType;
...
public abstract class FFmpegMediaConverter<TProvider> : IMediaConverter where TProvider : IMediaProvider
{
    ...
    public virtual async Task ExecuteAsync(CancellationToken cancellationToken = default)
    {
        ...
        await ffmpeg.ConvertAsync(builder =>
        {
            ...
            foreach (var file in definition.Files)
            {
                ...
                if (definition.Codec.ConstantRateFactor.HasValue)
                    builder.ConstantRateFactor(definition.Codec.ConstantRateFactor.Value);
    ...
    /// </summary>
    protected virtual StreamId GetStreamId(StreamInfo stream)
    {
        return StreamId.Index(stream.Id);
    }
    ...
    /// </summary>
    /// <returns></returns>
    protected virtual bool RequireFullProbeSize()
    {
        return false;
    }
    ...
    {
        long length = 0;
        foreach (var segmentId in Parameter.Definition.Identifier.SegmentIds) length += GetSegmentFilesize(segmentId);
