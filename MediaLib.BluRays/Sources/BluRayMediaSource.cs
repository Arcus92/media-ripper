...
public class BluRayMediaSource : IMediaSource
{
    ...
    private MediaInfo BuildMediaInfo()
    {
        ...
        foreach (var item in _playlist.Items) duration += BluRay.TimeSpanFromBluRayTime(item.Duration);
        ...
        foreach (var stream in streams.Where(stream => stream.Type == StreamType.Subtitle))
        {
            ...
            if (counter == 1) stream.Flags |= StreamFlags.Forced;
        ...
        var chapters = ChapterInfo.FromTimestamps(
            ...
            _playlist.Marks.Select(mark =>
            {
                ...
                uint offset = 0;
                for (var i = 0; i < mark.PlayItemId; i++) offset += _playlist.Items[i].Duration;
