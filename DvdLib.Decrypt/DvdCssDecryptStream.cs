...
public class DvdCssDecryptStream : Stream
{
    ...
    public DvdCssDecryptStream(string devicePath, uint titleSetSector, uint cellStartSector, uint cellEndSector)
    {
        ...
        if (!_dvdCss.Open(devicePath)) throw new IOException($"DvdCss couldn't open device: {devicePath}");
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
        ...
        {
            // Detect end of cell
            if (_positionCurrent + _bufferOffset >= _positionEnd) break;
    ...
    /// <returns></returns>
    public static DvdCssDecryptStream Open(string devicePath, uint titleSetSector, uint cellStartSector,
        uint cellEndSector)
    {
