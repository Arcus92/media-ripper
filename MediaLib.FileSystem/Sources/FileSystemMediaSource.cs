...
public class FileSystemMediaSource : IMediaSource
{
    ...
    private MediaInfo BuildMediaInfo(string name, InputMetadata metadata)
    {
    ...
    private static StreamFlags MapStreamFlags(StreamMetadata streamMetadata)
    {
        var flags = StreamFlags.None;

        if (streamMetadata.IsDefault) flags |= StreamFlags.Default;
        if (streamMetadata.IsForced) flags |= StreamFlags.Forced;
