...
public static class FileRenamer
{
    ...
    {
        if (!TryGetRenameStrategy(renameMap, out var strategy)) return false;

        foreach (var entry in strategy) File.Move(entry.Filename, entry.NewFilename);
    ...
    {
        if (!TryRename(renameMap)) throw new IOException("Unable to rename files: Invalid rename strategy.");
    }
    ...
    private static bool TryGetRenameStrategy(Dictionary<string, string> input, out RenameStrategy strategy)
    {
        ...
        foreach (var (filename, newFilename) in normalizedInput)
        {
            ...
            if (!normalizedInput.ContainsKey(newFilename)) return false;
