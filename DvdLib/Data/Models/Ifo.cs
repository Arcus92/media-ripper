...
public class Ifo
{
    ...
    private const int DvdBlockLength = 2048;
    ...
    private void ReadVmg(BigEndianBinaryReader reader)
    {
        ...
        if (vmg.TtSrpt > 0)
        {
            reader.SkipTo(vmg.TtSrpt * DvdBlockLength);
            TtSrpt = Models.TtSrpt.FromReader(reader);
        }
    ...
    private void ReadVts(BigEndianBinaryReader reader)
    {
        ...
        if (vts.VtsPttSrpt > 0)
        {
            reader.SeekTo(vts.VtsPttSrpt * DvdBlockLength);
            VtsPttSrpt = Models.VtsPttSrpt.FromReader(reader);
        }
        if (vts.VtsPgcit > 0)
        {
            reader.SeekTo(vts.VtsPgcit * DvdBlockLength);
